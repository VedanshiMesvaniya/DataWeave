...
- **`src/components/`**: Reusable UI components:
  ...
  - `VoiceWaveform.jsx`: A full-width, filled gradient waveform blob (Siri-style) shown in place of the text input while recording. Maintains a small scrolling history buffer of loudness (RMS of `useVoiceRecorder`'s `getWaveform()` samples, eased frame-to-frame so it swells/settles smoothly), then builds a closed, smoothed SVG path — mirrored top and bottom envelopes around a center line — via a lightweight "quadratic through midpoints" curve rather than a full Catmull-Rom spline, cheap enough to recompute every frame. Fill is a horizontal gradient built entirely from theme variables (`--blob-a`/`--blob-b`/`--blob-c`/`--accent`/`--text-secondary` — the same palette used for the app's ambient background blobs), with a second, blurred copy of the same path underneath for a soft glow layer. Uses a fixed viewBox with `preserveAspectRatio="none"` to stretch to any composer width without a `ResizeObserver`. The `d` attribute on both `<path>`s is written directly (not via React state) inside a `requestAnimationFrame` loop, so a 50-60 fps audio readout doesn't force component re-renders.
