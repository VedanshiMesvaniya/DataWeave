...
- **`src/components/`**: Reusable UI components:
  ...
  - `VoiceWaveform.jsx`: A full-width, glowing gradient waveform line shown in place of the text input while recording — an SVG path traced from the mic's actual time-domain waveform (via `useVoiceRecorder`'s `getWaveform()`, backed by a Web Audio `AnalyserNode`'s `getByteTimeDomainData`), so it genuinely moves up and down with voice pitch/loudness rather than looping a canned animation. A second, blurred copy of the same path renders beneath the crisp stroke for a soft glow. The stroke's gradient is built entirely from theme variables (`--blob-a`/`--blob-b`/`--blob-c`/`--accent`/`--text-muted` — the same palette used for the app's ambient background blobs), so it's on-brand and switches automatically for light/dark with no hardcoded colors. Uses a fixed viewBox with `preserveAspectRatio="none"` to stretch to any composer width without a `ResizeObserver`, and `vector-effect="non-scaling-stroke"` to keep line thickness constant under that non-uniform scale. The `d` attribute on both `<path>`s is written directly (not via React state) inside a `requestAnimationFrame` loop, so a 50-60 fps audio readout doesn't force component re-renders.
  - `SelectionReplyPopup.jsx`: Select-to-reply — listens for `selectionchange` events, and when the selection lands inside the scrollable message list (passed in as `containerRef`) and isn't collapsed, shows a small "Reply" pill positioned at the top of the selection's bounding rect. Renders as a child of `.message-stream` itself (which has `position: relative`) so the popup scrolls naturally with the content — its `top`/`left` are computed from `getBoundingClientRect()` plus the container's current `scrollTop`, converting a viewport-relative selection position into one relative to the scrollable content. On click (captured on `mousedown`, since the browser's own text selection collapses before a `click` event would fire), it hands the selected text to an `onReply` callback and clears the browser selection.
