# DataWeave Deep Architecture & Context Map

This document is the **definitive, granular guide** to the DataWeave project. Paired with `CLAUDE.md` (which dictates coding standards), this file explains *how everything works, from the smallest utility functions to the macro system architecture*. 
...
DataWeave is an enterprise-grade Retrieval-Augmented Generation (RAG) system built with a single, aggressive constraint: **It must operate entirely on forever-free-tier APIs while rivaling paid systems in accuracy.**
...
├── DataWeave_UI/   # React frontend source (Vite)
...
`data/` is `.gitignore`d, so its contents are never reviewed in a code diff — a debug script that once wrote a one-off file there can leave it behind indefinitely with nothing to notice. `tests/test_data_dir_hygiene.py` closes that gap: it enumerates every file/directory the current code actually writes under `data/` (this file, `pipeline_metrics.py`, `metadata_store.py`, and the upload/processed/inbox dirs from `config.py`) and fails if anything else shows up. Adding a new persisted file means adding it to that allow-list in the same change.
...
`POST /api/transcribe` accepts a multipart audio upload (webm/ogg/wav/mp3/m4a/mp4/flac), writes it to a temporary file, enforces `max_audio_upload_mb`, transcribes it, and returns `{"text": "..."}`. It sits at the same architectural layer as `upload.py` and `query.py` but talks only to `src/core/speech.py`. The frontend inserts the returned text into the existing composer input — the transcript is just another string until the user presses Send, at which point it flows through the ordinary chat/query path like typed text.

**No audio is persisted, on either side of the request.** The temp file is created with `delete=False` only so it can be reopened by faster-whisper by path, and is unconditionally removed via `os.unlink()` in a `finally` block that runs whether transcription succeeds, fails, or raises — it never lands in `data/` or any other durable location. On the frontend, the recorded `Blob` is a local variable inside `useVoiceRecorder.js`'s `onstop` handler; once the upload resolves (or rejects), nothing in the app still references it, so it's immediately eligible for garbage collection. Net effect: the actual recording exists for roughly the lifetime of a single request and nowhere longer.
...
## 5. Frontend Architecture (`DataWeave_UI/`)
...
- **`src/components/`**: Reusable UI components:
  - `Chat.jsx`: Main chat wrapper for messages and input. Also owns the select-to-reply flow: renders `SelectionReplyPopup` inside the message list, holds the `quotedReply` state it feeds, shows a dismissible quote chip above the composer, and — on send — prepends the quoted text as a markdown blockquote ahead of the typed question before calling `sendPrompt`, so the quoting logic lives entirely at this level with no changes to the store, API layer, or backend.
  ...
  - `VoiceWaveform.jsx`: A bar-style live equalizer shown next to the "Listening..." label while recording, matching a classic voice-recorder look rather than a single smooth line. Maintains a small scrolling history buffer of loudness (RMS of `useVoiceRecorder`'s `getWaveform()` samples, eased frame-to-frame via `SMOOTHING` so bars rise/settle smoothly instead of jittering with raw mic noise), and writes each bar's height/opacity directly to the DOM inside a `requestAnimationFrame` loop rather than via React state, since this repaints 50-60x/sec. Color is a single theme accent (`var(--accent)`), varying only in opacity/height with loudness, rather than a multi-hue gradient.
  - `SelectionReplyPopup.jsx`: Select-to-reply — listens for `selectionchange` events, and when the selection lands inside the scrollable message list (passed in as `containerRef`), isn't collapsed, and isn't anchored inside a `.message--user` bubble (quoting your own messages back isn't useful, so those are ignored), shows a small "Reply" button positioned at the top of the selection's bounding rect. Renders as a child of `.message-stream` itself (which has `position: relative`) so the popup scrolls naturally with the content — its `top`/`left` are computed from `getBoundingClientRect()` plus the container's current `scrollTop`, converting a viewport-relative selection position into one relative to the scrollable content. On click (captured on `mousedown`, since the browser's own text selection collapses before a `click` event would fire), it hands the selected text to an `onReply` callback and clears the browser selection.
...
- **`src/utils/`**:
  ...
  - `useVoiceRecorder.js`: React hook wrapping the browser's `MediaRecorder` and Web Audio APIs. `startRecording`/`stopRecording`/`toggleRecording` drive a small state machine (`idle` → `recording` → `transcribing` → `idle`); while recording, a `MediaStreamSource` feeds an `AnalyserNode` (connected only to the analyser, never to `audioContext.destination`, so nothing is played back) that `getWaveform()` samples on demand — reading `getByteTimeDomainData` and normalizing it to -1..1, which `VoiceWaveform.jsx` turns into RMS-based bar heights. `cancelRecording()` stops the recorder like `stopRecording()` but sets a `cancelledRef` flag first, which the `onstop` handler checks before doing anything else — if set, the clip is thrown away with no upload and no transcription request at all, and the flag resets for the next recording. On a normal stop, the recorded `Blob` is sent to `transcribeAudio()` (`api.js` → `POST /api/transcribe`) and the resulting text is handed back via an `onTranscript` callback; the `Blob` and the `AudioContext` are both released immediately afterwards (`cleanupAudioAnalysis()` closes the context, and the blob simply falls out of scope). `Chat.jsx`'s `onTranscript` handler types the transcript into the composer character-by-character (a small `setInterval`-driven reveal, not a direct assignment) rather than dropping it in all at once, and sets an `isRevealingVoice` flag `InputBox.jsx` uses to show a small loading icon next to the textarea during that reveal — voice input never bypasses the normal text box.
...
1. User runs `dataweave ingest Model_Card.pdf`.
