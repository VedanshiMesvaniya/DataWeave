...
- **`src/components/`**: Reusable UI components:
  ...
  - `VoiceWaveform.jsx`: A 5-bar live equalizer shown next to the mic button while recording, reflecting actual mic input (via `useVoiceRecorder`'s `getLevels()`, backed by a Web Audio `AnalyserNode`) rather than a canned animation. Bar heights are written directly to the DOM inside a `requestAnimationFrame` loop, not via React state, so a 30-60 fps audio readout doesn't force component re-renders.
