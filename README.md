...
- Voice input: a microphone button beside Send replaces the text box with a full-width glowing gradient waveform line while recording (moves with your voice's actual pitch/loudness, colored from the app's own theme palette), with a cancel button to discard the recording without transcribing; transcribes locally when you stop, and drops the text into the composer for review before sending — the audio itself is discarded immediately after transcription, on both ends
- Select-to-reply: highlight any text in a message (like ChatGPT/Claude) to get a "Reply" pill — clicking it quotes that exact passage into the composer as a blockquote ahead of your next question, so the model sees precisely what you're asking about

### Voice input (speech-to-text)
Clicking the mic icon next to Send records audio in the browser (`MediaRecorder`, no extra SDK) and replaces the text input with a full-width, glowing gradient line — a continuous stroke that traces the mic's actual waveform shape via a Web Audio `AnalyserNode`, so it genuinely moves with your voice's pitch and loudness rather than looping a canned animation (`useVoiceRecorder.js` / `VoiceWaveform.jsx`). The gradient uses the app's own theme colors (`--blob-a`/`--blob-b`/`--blob-c`/`--accent`, the same palette behind the ambient background blobs elsewhere), so it's on-brand and switches automatically for light/dark — nothing is hardcoded. It's for visual feedback only and is never played back or saved. While recording, a cancel (✕) button sits next to the mic so you can discard the clip entirely — no upload, no transcription — instead of always having to stop-and-transcribe. Clicking the mic again (or letting the recorder auto-stop) uploads the clip to `POST /api/transcribe`. The backend transcribes it locally with [faster-whisper](https://github.com/SYSTRAN/faster-whisper) (`large-v3-turbo` by default, CPU + int8 out of the box — no API key, nothing leaves your machine) and returns plain text, which is inserted into the existing input box exactly as if it had been typed. The transcript is never sent straight to the RAG pipeline: the user can edit or delete it before pressing Send, and `/api/query` has no idea whether a question came from the keyboard or the mic.
...
Highlighting text inside any message shows a small "Reply" pill above the selection (`SelectionReplyPopup.jsx`), the same pattern ChatGPT and Claude use. Clicking it drops the exact selected text into a quote chip above the composer; when you send your next message, that quote is prepended as a markdown blockquote (`> quoted text`) ahead of your typed question. This means the model sees exactly which passage the follow-up refers to — no backend or pipeline changes were needed, since `/api/query` and the chat endpoints just receive a slightly longer, already-contextualized question, the same way they'd receive it if you'd retyped the quoted passage yourself before asking.
