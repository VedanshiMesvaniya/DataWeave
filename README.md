...
14. **SQL retrieval (optional)** — a parallel text-to-SQL stage answers questions against a live SQLite or MySQL database. It's grounded in the real data, not just the schema: `EntityValueResolver` samples real distinct values from name/category/status-like columns and feeds any that match the question into the prompt as exact values to filter on, so a question naming a specific customer, product, or category resolves correctly instead of the model guessing at spelling/casing. Every generated query is parsed and validated as a read-only `SELECT` via `sqlglot` and capped at a maximum row count before it ever touches the database
...
- Voice input: a microphone button beside Send shows a live "Listening..." state (bar-equalizer waveform reacting to your actual voice, colored from the app's own theme accent) while recording, with a cancel button to discard the recording without transcribing, and a character-by-character reveal of the transcript into the composer once you stop — the audio itself is discarded immediately after transcription, on both ends
- Select-to-reply: highlight any text in an assistant message (like ChatGPT/Claude) to get a "Reply" button — clicking it quotes that exact passage into the composer as a blockquote ahead of your next question, so the model sees precisely what you're asking about (not offered on your own messages)

### Voice input (speech-to-text)
Clicking the mic icon next to Send starts a "Listening..." state: a small bouncing equalizer icon and label appear on the left, and a bar-style waveform (driven by a Web Audio `AnalyserNode` reading your actual mic loudness, in the app's own `--accent` color) fills the rest of the row (`useVoiceRecorder.js` / `VoiceWaveform.jsx`) — it's for visual feedback only and is never played back or saved. The mic button itself becomes a stop button (outlined in red, pulsing) while recording, and a cancel (✕) button sits next to it so you can discard the clip entirely — no upload, no transcription — instead of always having to stop-and-transcribe. Clicking the stop button (or letting the recorder auto-stop) uploads the clip to `POST /api/transcribe`. The backend transcribes it locally with [faster-whisper](https://github.com/SYSTRAN/faster-whisper) (`large-v3-turbo` by default, CPU + int8 out of the box — no API key, nothing leaves your machine) and returns plain text, which is typed into the composer character-by-character (with a small loading indicator while it plays out) rather than dropped in all at once. The transcript is never sent straight to the RAG pipeline: the user can edit or delete it before pressing Send, and `/api/query` has no idea whether a question came from the keyboard or the mic.

**The audio is never kept.** On the frontend, the recorded clip lives only as a local variable for the duration of the upload — nothing in the app stores or replays it, and it's eligible for garbage collection the moment the request finishes. On the backend, `POST /api/transcribe` writes the clip to a temp file only for the duration of transcription and deletes it in a `finally` block that always runs, success or failure, before the response is returned — no audio is ever written to `data/` or any other durable storage. Cancelling a recording never reaches the backend at all — no request is sent.
...
### Select-to-reply (quote a passage into your next question)
Highlighting text inside an assistant message shows a small "Reply" button above the selection (`SelectionReplyPopup.jsx`), the same pattern ChatGPT and Claude use — selections inside your own messages are ignored, since quoting yourself back isn't useful. Clicking it drops the exact selected text into a quote chip above the composer; when you send your next message, that quote is prepended as a markdown blockquote (`> quoted text`) ahead of your typed question. This means the model sees exactly which passage the follow-up refers to — no backend or pipeline changes were needed, since `/api/query` and the chat endpoints just receive a slightly longer, already-contextualized question, the same way they'd receive it if you'd retyped the quoted passage yourself before asking.
...
│   ├── src/
│   │   ├── components/         # Chat.jsx, Sidebar.jsx, Message.jsx, Header.jsx, VoiceWaveform.jsx, SelectionReplyPopup.jsx, ...
